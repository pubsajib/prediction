...
class matchTop {
	public static function render($attr) {
		...
		$attr = shortcode_atts($defaults, $attr, 'matchTop');
		$html  = '';
		$predictors = self::getPredictorsList();
		$ranking = self::getRakingFor('match', false, $predictors);
	...
	public static function getRakingFor($ratingType='match', $tournamentID=false, $predictors='', $minItemToPredict=100, $itemGrace=10, $minParticipationRate=65) {
		$top3 = 3;
		$top10 = 10;
		$ranking = [];
		$users = [];
...
// 		$lifeTimeParticipationCriteria = 1;
	...
	public static function html($predictors, $ranking, $attr) {
		...
		if ($ranking['all']) {
			$html = '';
			$html .= '<div class="predictorListWrapper tossmatch match-equal"><div class="equalAll">';
			foreach ($ranking['all'] as $rankID => $rank) {
				if ($rankID >= $attr['number']) break;
				$user = $predictors[$rank['id']];
				$ratingIcon = '';
				...
				$html .= '<div id="predictor_'. $user->ID .'" class="predictorContainer author-profile-card'. $rank['class'] .'">';
                    ...
                    if (!empty($UP['avg'])) {
                        $html .= '<table class="table top-accuracy">';
                            $html .= '<tr>';
								$html .= '<td><small>Accuracy <!--(' . $UP['avg']['match']['participated'] . ')--> </small><br>' . $UP['avg']['match']['rate'] . '%<br><small class="last"><span class="green">'. $UP['avg']['match']['correct'] . '</span>/<span class="red">'. $UP['avg']['match']['incorrect'] . '</span></small></td>';
								$html .= '<td>';
									$t20 = tournamentData($user->ID, 267);
										if (isset($t20['avg'])) {
											$html .= '<small>T20 (' . $t20['avg']['match']['participated'] . ') </small><br>' . $t20['avg']['match']['rate'] . '%<br><small class="last"><span class="green">'. $t20['avg']['match']['correct'] . '</span>/<span class="red">'. $t20['avg']['match']['incorrect'] . '</span></small>';
										}
								$html .= '</td>';
								$html .= '<td>';
									$odi = tournamentData($user->ID, 266);
										if (isset($odi['avg'])) {
											$html .= '<small>ODI (' . $odi['avg']['match']['participated'] . ') </small><br>' . $odi['avg']['match']['rate'] . '%<br><small class="last"><span class="green">'. $odi['avg']['match']['correct'] . '</span>/<span class="red">'. $odi['avg']['match']['incorrect'] . '</span></small>';
										}
								$html .= '</td>';
								$html .= '<td>';
									$test = tournamentData($user->ID, 265);
										if (isset($test['avg'])) {
											$html .= '<small>Test (' . $test['avg']['match']['participated'] . ') </small><br>' . $test['avg']['match']['rate'] . '%<br><small class="last"><span class="green">'. $test['avg']['match']['correct'] . '</span>/<span class="red">'. $test['avg']['match']['incorrect'] . '</span></small>';
										}
								$html .= '</td>';
                            $html .= '</tr>';
                        $html .= '</table>';
		    		...
		    		// PROFILE INFORMATION
					$html .= '<div class="profile-info">'. profileInfo($user, false, $ratingIcon) .'</div>';
					...
					$html .= '<div class="sliderFooter">';
						...
						if (isset($t20['avg'])) {
							$html .= "<td>T20</td>";
							$html .= "<td>" . round($t20['avg']['match']['rate']) . "% (" . $t20['avg']['match']['participated'] . ")</td>";
							$html .= '</tr>';
						}
						$odi = tournamentData($user->ID, 266);
						if (isset($odi['avg'])) {
							$html .= "<td>ODI</td>";
							$html .= "<td>" . round($odi['avg']['match']['rate']) . "% (" . $odi['avg']['match']['participated'] . ")</td>";
							$html .= '</tr>';
						}
						$test = tournamentData($user->ID, 265);
						if (isset($test['avg'])) {
							$html .= "<td>Test</td>";
							$html .= "<td>" . round($test['avg']['match']['rate']) . "% (" . $test['avg']['match']['participated'] . ")</td>";
							$html .= '</tr>';
						}
						$html .= '</table>';
						$html .= '</div>';
...
add_shortcode('matchTop', ['matchTop', 'render']);