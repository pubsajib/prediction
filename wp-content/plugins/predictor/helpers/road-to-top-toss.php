<?php 
function roadToTopToss($user) {
    $min = ['avg' => 50, 'match' => 100, 'engagement' => 50];
    RoadToTopToss::render($user, 'toss', $min);
}
class RoadToTopToss {
    static function render($user='', $type='all', $min=[]) {
        ...
        if ( !in_array( 'predictor', (array) $user->roles ) ) echo "Not a predictor";
    		...
    		$html .= '<div class="login-profile">';
        		...
        		if ($lifeTimeEvents) {
        			...
        			$html .= '<div class="item">
    					...
    					<div class="additional">
    						<span>Your minimal engagement should be <strong>50%</strong></span>
    					</div>
        			</div>';
        		}
    		...
    		$html .= '</div>';
			// $html .= '<div class="notice"><span class="small"><strong>'. $lifeTimeEvents .'</strong> Tosses published since you join as an expert and <strong>'. $toalPublished .'</strong> Tosses published since the system was born on 1st Jan 2019.</span></div>';
            echo $html;
